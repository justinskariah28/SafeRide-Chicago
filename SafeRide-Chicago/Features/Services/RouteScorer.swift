...
enum RouteScorer {
    ...
    static func analyze(
        ...
        let usesHalsted = routeUsesStreetName(
            route: route,
            streetNames: [
                "Halsted",
                "S Halsted",
    ...
    private static func applyCrowdingScore(
        ...
        var hasVerifiedCrowdingData = false
        
        /*
         Arrigo Park is one of the quietest available paths.
         Strongly prefer it when the user selects Avoid crowded
         areas. The separate lighting scorer still strongly
         penalizes it when Well-lit streets is also selected.
         */
        if analysis.passesThroughArrigoPark {
            score += 100
            hasVerifiedCrowdingData = true
            
            reasons.insert(
                "Uses the quieter walking paths through Arrigo Park.",
                at: 0
            )
        }
        ...
        if analysis.usesTaylorStreet {
            ...
            reasons.append(
                "Uses Taylor Street, which is often more crowded."
            )
        }
        
        if analysis.usesHalstedStreet {
            score -= 70
            hasVerifiedCrowdingData = true
            
            reasons.append(
                "Uses Halsted Street, which is often more crowded."
            )
        }
        ...
        if let crowding = analysis.averageCrowdingScore {
            score += crowding * 30
            hasVerifiedCrowdingData = true
            
            if crowding >= 0.8 &&
                !analysis.passesThroughArrigoPark {
